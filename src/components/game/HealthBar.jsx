...
export function FactionBadge({ type }) {
  return (
    <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold text-white uppercase tracking-wide" style={{ backgroundColor: FACTION_COLORS[type] || "#888" }}>
      {type}
    </span>
  );
}
...
export function SognatoreImage({ s, className = "w-16 h-16" }) {
  if (s.img) {
    return (
  ...
  return <div className={`${className} flex items-center justify-center text-4xl`}>{s.emoji || "❓"}</div>;
}